        ...
        <div class="input-group input-group-lg">
            <span class="input-group-text rounded-0 bg-transparent p-0" id="search_page">
                <select wire:model="what_search" class="form-select rounded-0 m-0 w-100 h-100">
                    ...
                    <option value="mediaType">Search For Media Type</option>
                </select>
            </span>
                ...
                @if (count($pageArray) > 0)
                    @foreach ($pageArray as $page)
                        <tr>
                            ...
                            <td>{{ strlen($page->subChapter) > 30 ? substr($page->subChapter, 0, 20) . '...' : $page->subChapter }}
                            </td>
                            <td>{{ $page->category }}</td>
                            <td>
                                <a href="{{ $page->videoUrl }}" class="btn btn-success rounded-0">Download</a>
                            </td>
                            <td>{{ $page->mediaType }}</td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <a type="button"
                                        href="{{ env('APP_URL') . '/admin/dashboard/manage-videos/'. $page->id }}"
