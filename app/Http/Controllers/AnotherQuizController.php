...
use App\Http\Resources\AnotherQuizResource;
...
class AnotherQuizController extends Controller
{
    public function index() {
        if (Quiz::all() === null) {
            ...
            return AnotherQuizResource::Collection(Quiz::paginate(1000));
        }
    }

    public function show($id) {
        if (Quiz::where('id', $id)->exists()) {
            return new AnotherQuizResource(Quiz::findOrFail($id));
        }
        else {
            return response([
                'response' => 'Quiz questions not found'
    ...
    public function showQuizCategory(Request $request) {
        ...
        return response([
            'response' => AnotherQuizResource::collection(Quiz::where("category", "LIKE", "%{$request->category}%")->paginate(100))
        ], 201);
    }
    ...
    public function showQuizChapter(Request $request) {
        ...
        if ($request->forWhat == 'subchapter') {
            return response([
                'response' => AnotherQuizResource::collection(Quiz::where("subChapter", "LIKE", "%{$request->subChapter}%")->get())
            ], 201);
        }
        else {
            $validator = \Validator::make($request->all(), [
            ...
            return response([
                'response' => AnotherQuizResource::collection(Quiz::where("chapter", "LIKE", "%{$request->chapter}%")->get())
            ], 201);
        }
    }
